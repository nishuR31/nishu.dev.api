...
export class AuthController {
  static async register(req: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      return sendSuccess(reply, "Developer registered successfully.", 200, {
        id: user.id,
        email: user.email,
      });
    } catch (error) {
  ...
  static async login(req: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      if (user.is2FAEnabled) {
        ...
        const sanitizedToken = data.token2FA.replace(/\s+/g, "");
  ...
  static async generatePasskeyOptions(req: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      let dynamicRpID = rpID;
      if (reqOrigin) {
        try {
          dynamicRpID = new URL(reqOrigin).hostname;
        } catch (e) {}
      }
    ...
    try {
      ...
      try {
        verification = await verifyRegistrationResponse({
          ...
          expectedRPID: [rpID, "localhost", dynamicRpID],
        });
      } catch (error: any) {
  ...
  static async verifyPasskeyAuthentication(req: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      const responseBuffer = Buffer.from(response.id, "base64url");
      const passkey = user.passkeys.find(
        (k) => Buffer.compare(k.credentialID, responseBuffer) === 0,
      );
      if (!passkey) {
  ...
  static async getPasskeys(req: FastifyRequest, reply: FastifyReply) {
    try {
      const userPayload = req.user as { id: string };
      const user = await prisma.user.findUnique({
        where: { id: userPayload.id },
        include: {
          passkeys: { select: { id: true, createdAt: true, transports: true } },
        },
      });
      if (!user) return sendError(reply, "User not found", 404);
      return sendSuccess(reply, "Passkeys retrieved", 200, user.passkeys);
    } catch (error: any) {
      return sendError(reply, "Failed to fetch passkeys", 500, error.message);
    }
  }
