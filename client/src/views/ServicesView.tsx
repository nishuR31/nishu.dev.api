...
export default function ServicesView() {
  ...
  const onSubmit = async (data: ServiceFormData) => {
    try {
      if (editingId) {
        await axios.put(`/api/portfolio/services/${editingId}`, data, {
  ...
  const toggleVisibility = async (service: any) => {
    try {
      ...
      await axios.put(`/api/portfolio/services/${service.id}`, payload, {
        withCredentials: true,
      });
      await fetchServices();
    } catch (error) {
  ...
  const handleBulkSave = async (parsedData: any) => {
    try {
      await axios.post("/api/portfolio/services/bulk", parsedData, {
        withCredentials: true,
      });
      await fetchServices();
    } catch (err) {
