...
export default function SettingsView() {
  ...
  const onSettingsSave = async (data: any) => {
    try {
      ...
      showToast(
        "Failed to save settings. Please check your developer authorization.",
        "error",
      );
    }
  };
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      handleSettingsSubmit((data) => {
                        onSettingsSave({ ...data, maintenanceMode: newValue });
                      })();
                    }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                onClick={async () => {
                  try {
                    const res = await axios.post(
                      "/api/portfolio/social/sync-stats?force=true",
                    );
                    showToast(
                      "Social stats successfully synced from external APIs!",
                      "success",
                    );
                  } catch (e: any) {
                    showToast(
                      "Failed to sync stats: " + (e.response?.data?.message || e.message),
                      "error",
                    );
                  }
                }}
          ...
          <div className="space-y-8">
            ...
            <div className="p-8 glass-panel rounded-3xl hover-lift">
              ...
              <div className="mt-4">
                <PasskeysManager onMessage={showToast} />
              </div>
            </div>
          </div>
        </div>
