...
export default function EducationView() {
  ...
  const onSubmit = async (data: EducationFormData) => {
    try {
      ...
      if (editingId) {
        await axios.put(`/api/portfolio/education/${editingId}`, payload, {
  ...
  const toggleVisibility = async (edu: any) => {
    try {
      ...
      await axios.put(`/api/portfolio/education/${edu.id}`, payload, {
        withCredentials: true,
      });
      await fetchEducation();
    } catch (error) {
  ...
  const handleBulkSave = async (parsedData: any) => {
    try {
      await axios.post("/api/portfolio/education/bulk", parsedData, {
        withCredentials: true,
      });
      await fetchEducation();
    } catch (err) {
          ...
          >
            ...
            <div className="flex-1 relative z-10">
              ...
              <div className="flex flex-col gap-2 mt-4 text-xs font-semibold text-[var(--muted-foreground)]">
                ...
                {edu.grade && (
                  <span className="inline-block px-2 py-1 rounded bg-[var(--background)]  self-start">
                    Grade: {edu.grade}
                  </span>
                )}
              </div>
            ...
            >
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-[var(--background)]  rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all font-medium"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    type="date"
                    {...register("startDate")}
                    className="w-full bg-[var(--background)]  rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all font-medium [color-scheme:dark]"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-bold opacity-80">
