...
export default function ExperiencesView() {
  ...
  const onSubmit = async (data: ExperienceFormData) => {
    try {
      ...
      if (editingId) {
        await axios.put(`/api/portfolio/experiences/${editingId}`, payload, {
          withCredentials: true,
        });
      } else {
        await axios.post("/api/portfolio/experiences", payload, {
          withCredentials: true,
        });
      }
  ...
  const toggleVisibility = async (exp: any) => {
    try {
      ...
      await axios.put(`/api/portfolio/experiences/${exp.id}`, payload, {
        withCredentials: true,
      });
      await fetchExperiences();
    } catch (error) {
  ...
  const handleBulkSave = async (parsedData: any) => {
    try {
      await axios.post("/api/portfolio/experiences/bulk", parsedData, {
        withCredentials: true,
      });
      await fetchExperiences();
    } catch (err) {
    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-700 pb-24 md:pb-0">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
        {experiences.map((exp) => (
          <div
            key={exp.id}
            className={`glass-panel p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover-lift duration-300 relative overflow-hidden group ${exp.visible === false ? "opacity-50 grayscale-[0.5]" : ""}`}
          >
            ...
            <p className="text-xs sm:text-sm opacity-80 mb-4 sm:mb-6 leading-relaxed line-clamp-3">
              {exp.description}
            </p>

            <div className="grid grid-cols-2 gap-3 sm:gap-4 pt-3 sm:pt-4 border-t border-[var(--border)]">
              <div>
                <h4 className="text-[10px] sm:text-xs uppercase tracking-widest opacity-50 font-bold mb-1 sm:mb-2">
                  Technologies
                </h4>
              ...
              <div>
                ...
                <div className="text-[10px] sm:text-xs opacity-80">
                  {exp.responsibilities?.length || 0} Listed
                </div>
              </div>
            </div>
          </div>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="glass-panel w-full max-w-3xl rounded-[2rem] shadow-2xl my-auto animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-white/5 flex items-center justify-between sticky top-0 glass-header rounded-t-[2rem] z-10">
              <h3 className="text-2xl font-bold">
                {editingId ? "Edit" : "New"} Experience
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  ...
                  <input
                    required
                    {...register("position")}
                    className="w-full bg-[var(--background)]  rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all font-medium"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-[var(--background)]  rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all font-medium resize-none"
                  placeholder="High-level description of your role..."
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-[var(--background)]  rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all font-medium resize-none"
                  placeholder="Led a team of 5, Increased performance by 40%..."
                />
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <label
                  htmlFor="isCurrent"
                  className="text-sm font-bold opacity-80 cursor-pointer"
                >
