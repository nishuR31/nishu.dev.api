...
export default function ProjectsView() {
  ...
  const handleBulkSave = async (parsedData: any) => {
    try {
      await axios.post("/api/portfolio/projects/bulk", parsedData, {
    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-700 pb-24 md:pb-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
        {projects.map((p) => (
          <div
            key={p.id}
            className={`p-4 sm:p-6 glass-panel rounded-2xl sm:rounded-3xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover-lift duration-300 flex flex-col justify-between group ${p.visible === false ? "opacity-50 grayscale-[0.5]" : ""}`}
          >
            <div>
              <div className="w-full h-28 sm:h-40 bg-[var(--background)] rounded-xl sm:rounded-2xl mb-3 sm:mb-4 overflow-hidden ">
                {p.image ? (
                  <img
                    src={p.image}
                    alt={p.title}
                  ...
                  <div className="w-full h-full flex items-center justify-center opacity-20">
                    <FolderGit2 className="w-10 h-10 sm:w-12 sm:h-12" />
                  </div>
                )}
              </div>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-start sm:items-center justify-center overflow-y-auto">
          <div className="glass-panel rounded-t-[1.5rem] sm:rounded-[2rem] w-full max-w-4xl shadow-2xl my-0 sm:my-6 animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200 min-h-0">
            <div className="p-4 sm:p-6 border-b border-white/5 flex items-center justify-between sticky top-0 glass-header rounded-t-[1.5rem] sm:rounded-t-[2rem] z-10">
            ...
            <form
              ...
              className="p-4 sm:p-6 max-h-[75vh] sm:max-h-[70vh] overflow-y-auto scroll-touch"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div className="space-y-3 sm:space-y-4">
                  <div>
                    <label className="block text-xs sm:text-sm font-semibold mb-1 sm:mb-1.5 opacity-80">
                      Title *
                    </label>
                    <input
                      required
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 pt-6 sm:pt-8 mt-4 sm:mt-6 border-t border-[var(--border)]">
                <button
                  type="button"
                  onClick={() => setIsFormOpen(false)}
                  className="px-5 sm:px-6 py-2.5 sm:py-3 font-semibold opacity-70 hover:bg-[var(--background)] rounded-xl transition-colors text-sm sm:text-base"
                >
                  Cancel
                </button>
                <button
