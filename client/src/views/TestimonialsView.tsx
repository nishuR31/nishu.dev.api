...
export default function TestimonialsView() {
  ...
  const onSubmit = async (data: TestimonialFormData) => {
    try {
      if (editingId) {
        await axios.put(`/api/portfolio/testimonials/${editingId}`, data, {
  ...
  const toggleVisibility = async (testimonial: any) => {
    try {
      ...
      await axios.put(`/api/portfolio/testimonials/${testimonial.id}`, payload, {
        withCredentials: true,
      });
      await fetchTestimonials();
    } catch (error) {
  ...
  const handleBulkSave = async (parsedData: any) => {
    try {
      await axios.post("/api/portfolio/testimonials/bulk", parsedData, {
        withCredentials: true,
      });
      await fetchTestimonials();
    } catch (err) {
            ...
            >
              <div className="space-y-1.5">
                ...
                <input
                  required
                  {...register("authorName")}
                  className="w-full bg-[var(--background)]  rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-pink-500 transition-all font-medium"
